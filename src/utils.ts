...
const utils = {
  ...
  generateRefprovider: (prefix: string = ""): string =>
    prefix ? `${prefix}_${crypto.randomUUID()}` : crypto.randomUUID(),
      ...
      if (checkIsLocal) {
        ...
        return new Promise((res) => {
          const response = {
            headers: {
              "content-type":
                mime.contentType(extname(url)) || "application/octet-stream",
            },
          };
  ...
  }): Promise<{ type: string; ext: string | any }> => {
